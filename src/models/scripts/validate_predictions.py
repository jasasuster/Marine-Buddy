...
from src.models.model import get_data, prepare_evaluate_data
...
def evaluate_model(sea_point_number):
  with mlflow.start_run(run_name=f"sea_point_{sea_point_number}", experiment_id="1", nested=True):
    ...
    X_final, y_final = prepare_evaluate_data(df, loaded_wave_scaler, loaded_other_scaler)
    production_X_final, production_y_final = prepare_evaluate_data(df, production_wave_scaler, production_other_scaler)
